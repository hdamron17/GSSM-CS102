...
public class ArrayUtil
{
    ...
    * @return Sorted array
    */
   public static int[] randomAscendingArray(int length) {
	   if(length > 250000000)
		   length = 250000000;
	   int[] a = new int[length];
	   a[0] = 1;
	   for(int k = 1; k < length && a[k] < Integer.MAX_VALUE - 4; k++) {
		   a[k] = a[k-1] + generator.nextInt(4);
	   }
	   for(int k = length - 1; a[k] == 0; k++) {
		   a[k] = Integer.MAX_VALUE;
	   }
	   return a;
   }
   
   public static int[] randomDescendingArray(int length) {
	   if(length > 250000000)
		   length = 250000000;
	   int[] a = new int[length];
	   a[length - 1] = 1;
